...
const Register = () => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            const res = await LedgerAPI.register(formData);
            if (res.token) {
                login(res.user, res.token);
            } else {
                // If backend requires manual login after registration
                navigate('/login', { state: { message: "Account created! Please sign in." } });
            }
        } catch (err) {
            console.error("Register Error:", err);
            setError(err.response?.data?.message || 'Failed to create account. Email may be in use.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden font-sans">
            ...
            <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 p-8 relative z-10">
                ...
                <form onSubmit={handleRegister} className="space-y-5">
                    ...
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Create Access'}
